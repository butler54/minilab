...
The pattern also ensures an LVMS-based default storage class for workloads that request no class (for example, the COO-managed Perses server). A least-privilege `CronJob` in the `lvms-config` chart (`minilab-lvmcluster-default-storage`) runs every 10 minutes and marks `lvms-loopback` as the cluster default only when no storage class is already the default; it never alters an existing foreign default and never changes explicit storage-class choices. It is idempotent and restores the default annotation if it is later removed. Disable it with `global.localStorage.defaultStorageClass.enabled: false`. Explicit `storageClassName` requests are unaffected.
