...
for chart in charts/lvms-config charts/observability-config; do helm lint "$root/$chart" -f "$root/values-global.yaml" >/dev/null; helm template "${chart##*/}" "$root/$chart" -f "$root/values-global.yaml" >/dev/null; done
