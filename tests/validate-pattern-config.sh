...
for chart in charts/lvms-config charts/observability-config charts/agent-sandbox charts/openshell-platform charts/openshell charts/openshell-policy charts/openshell-demo charts/cert-manager-config charts/ztwim-config; do helm lint "$root/$chart" -f "$root/values-global.yaml" >/dev/null; helm template "${chart##*/}" "$root/$chart" -f "$root/values-global.yaml" >/dev/null; done
