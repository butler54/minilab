...
for chart in charts/lvms-config charts/observability-config charts/openshell-extras charts/openshell-platform charts/openshell-policy charts/openshell-demo charts/cert-manager-config; do helm lint "$root/$chart" -f "$root/values-global.yaml" >/dev/null; helm template "${chart##*/}" "$root/$chart" -f "$root/values-global.yaml" >/dev/null; done
